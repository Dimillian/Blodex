...
export class DungeonScene extends Phaser.Scene {
  ...
  private readonly rng = new SeededRng(`run-${Date.now()}`);
  ...
  private computeWorldOriginAndBounds(): void {
    const corners = [
  ...
  private generateMonsterSpawnPoints(count: number): Array<{ x: number; y: number }> {
    ...
    while (points.length < count && candidates.length > 0) {
      ...
      const tooClose = points.some(
        (point) => Phaser.Math.Distance.Between(point.x, point.y, picked.x, picked.y) < 2.8
      );
      if (!tooClose) {
        points.push(picked);
      }
  ...
  private computePathTo(x: number, y: number): GridNode[] {
  ...
  private clampToWalkable(x: number, y: number): GridNode {
    const clamped = {
  ...
  private updateCombat(): void {
    ...
    if (target.state.health <= 0) {
      ...
      const nextDerived = deriveStats(
        ...
        Object.values(this.player.equipment).filter((item): item is ItemInstance => item !== undefined)
      );
      ...
      if (lootTable !== undefined) {
        const item = rollItemDrop(
          ...
          this.run.floor,
          this.rng,
          `${target.state.id}-${this.run.kills}`
        );
  ...
  private pickMonsterAt(x: number, y: number): MonsterRuntime | null {
